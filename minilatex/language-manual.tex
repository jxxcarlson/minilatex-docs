beginMetadata:
{
    ...
    "keyString": "minilatex: language manual a=jxxcarlson minilatex/language-manual.tex t=minilatex t=manual",
...
The  fragment \code{/g/} stands for \italic{guest}.  Using this link will take you to document 78, and you will be automatically logged in as guest.  You can also use the following more verbose method:

\begin{verbatim}
minilatex.lamdera.app/g/uuid:6f5a573d-5603-4470-8dc9-b0972997a6e6
\end{verbatim}

There very long string \code{6f5...6e6} is the UUID of the document. There are $2^{122} > 10^{36}$ such numbers, so the chance that any two chosen at random is very, very small.

The document number is on display in the footer of the app.  To reveal the UUID of a document, click on the \blue{Doc Tools} button (footer, lower left).

\subsection{Cross links}

You can make link in one MiniLaTeX document that points to another.
Here is a \xlink{78}{an example}. It is done like this:
...
One can use the \code{\bs{ilink}} macro to construct a kind of index of links that refer to other documents.  For an example of how this is done, please see \xlink{78}{these class notes} and look at the source. For best results, put the tag \italic{index} in the tag field (editor, upper right).  Then press the \blue{Tags} button.
