...
Since all clauses are proved in the same way, let consider only the third, which treats the case of the function \ccode{le-from-shift,} applied  to \ccode{b O}. The function value is the type
...
For the proof of the main proposition, two lemmas are required, \ccode{le-to-suc} and \ccode{le-from-inc-to.} The first of these id one of the elementary lemmas above. For the second, we first show the following:
...
The base case is \ccode{from(inc <>) ≡ suc(from <>).} Applying the rules for \ccode{inc} and \ccode{from,} one transforms this type to the equal type \ccode{suc 0 ≡ suc 0.} Therefore the type \ccode{le-from-inc <>} is inhabited by \ccode{refl.}
