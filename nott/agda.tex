...
To do so, we must show that the type \ccode{id-from-to} is inhabited.  This we do by constructing, via induction, a function of that type.  In the base case of \ccode{n = 0}, the type is
...
The first step is to observe that we have a set of elementary lemmas that follow immediately from the definitions.  Consider, the function \ccode{le-from-shift,} applied  to \ccode{b O} (third clause of \ccode{le-from-shift} below). This is the type
...
This type is inhabited by \ccode{refl.}  All the other proofs are the same.  To summarize: the lemmas below simply express elementary facts about the constructors.
...
For the proof of the main proposition, two lemmas are required, \ccode{le-to-suc} and \ccode{le-from-inc-to.}. The first of these is proved.  The statement of the second is

\begin{colored}[elm]
le-from-inc : ∀ ( b : Bin ) → from(inc b) ≡ suc(from b)
\end{colored}

As usual, we show that this type is inhabited via an inductive construction.  The base case is 

\begin{colored}[elm]
 from(inc <>) ≡ suc(from <>)
\end{colored}

Applying the rules for \ccode{inc} and \ccode{from}, one transforms this type to an equal type 

begin{colored}[elm]
suc 0 ≡ suc 0
\end{colored}

Therefore \ccode{le-from-inc <>,} which is type, is inhabited by \ccode{refl.}
